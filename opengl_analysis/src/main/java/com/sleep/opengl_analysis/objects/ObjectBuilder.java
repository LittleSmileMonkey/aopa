...
public class ObjectBuilder {
    ...
    static GenerateData createMattel(Geometry.Point center, float radius, float height, int numPoints) {
        int size = sizeOfCircleInVertices(numPoints) * 2 + sizeOfOpenCylinderInVertices(numPoints) * 2;
        final ObjectBuilder builder = new ObjectBuilder(size);
        //first generate the mattel base
        float baseHeight = height * 0.25f;
        final Geometry.Circle baseCircle = new Geometry.Circle(center.translateY(-baseHeight), radius);
        final Geometry.Cylinder baseCylinder = new Geometry.Cylinder(center.translateY(-baseHeight / 2f), radius, baseHeight);
        builder.appendCircle(baseCircle, numPoints);
        builder.appendOpenCylinder(baseCylinder, numPoints);

        //generate the mattel handle
        float handleHeight = height * 0.75f;
        float handleRadius = radius / 3f;
        final Geometry.Circle handleCircle = new Geometry.Circle(center.translateY(handleHeight * 0.5f), handleRadius);
        final Geometry.Cylinder handleCylinder = new Geometry.Cylinder(handleCircle.center.translateY(-handleHeight / 2f), handleRadius, handleHeight);
